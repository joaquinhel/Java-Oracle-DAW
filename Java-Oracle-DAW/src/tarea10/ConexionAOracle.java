...
import oracle.jdbc.pool.*;//Contiene las clases para realizar una conexión a oracle a través de JDBC
import java.sql.*; //Contiene las clases para conectarse via JDBC a una base de datos (cache u oracle)
...
public class ConexionAOracle {
     ...
     */
    public Connection construirConexion(String ipDelServidor, int puertoDelServidor, String sid, String usuario,
            String password) {
        try {
        ...
        } catch (SQLException ex) {  //Excepcion sino es posible realizar la conexión
     ...
     *
     * @param HR Nombre de la instancia. En el caso de Oracle XE el nombre de la
     * instancia de la base de datos es "XE".
     * @param usuario Usuario para conectarse, generalmente "HR".
     * @param password Password para conectarse, cuyo dato depende de como
     * tengas configurada la base de datos.
     ...
     */
    public Connection construirConexion(String HR,
            String usuario, String password) {
        return construirConexion("127.0.0.1", 1521, HR, usuario, password);
    }

    /**
     * Obtiene la conexión realizada a Oracle o null si todavía no se ha
     * conectado.
     *
